...
def run_single(degree: int, threads: int):
    """
    Run  mpiexec -n <threads> ./main <deegrees>
    ...
    cmd = ["mpiexec", "-n", str(threads), "-f", hostfile, EXE_PATH, str(degree)] #comment this line when on linux
    #cmd = ["mpirun", "-np", str(threads), EXE_PATH, str(degree)] #comment this line when on windows
    try:
    ...
    if gen_time is None or serial_time is None or parallel_time is None or speedup_time is None:
        print("\n[ERROR] Could not parse times from output:")
        print(output)
        raise RuntimeError("Failed to parse timing lines")

    if not results_match:
        print(f"\n[ERROR] Results did NOT match for degree={degree}, processes={threads}.")
        print("Output:\n", output)
        raise RuntimeError("Results mismatch")

    return gen_time, serial_time, parallel_time, speedup_time
...
def run_combo_and_stats(degree, threads, repeats):
    ...
    for it in range(repeats):
        try:
            gen_time, serial_time, parallel_time, speedup_time = run_single(degree, threads)
    ...
    speedup_mean = float(np.mean(speedup_times))
...
def run_experiments_write_stats():
    ...
    for degree in DEGREE_VALUES:
        ...
        for threads in my_threads:
            combo_counter += 1
            print(f"[INFO] Combo {combo_counter}/{total_combos} | degree={degree}, processes={threads}, repeats={my_repeats}")

            row = run_combo_and_stats(degree, threads, my_repeats)
            if row is None:
                print(f"[WARN] No successful samples for degree={degree}, processes={threads}. Skipping.")
                continue
